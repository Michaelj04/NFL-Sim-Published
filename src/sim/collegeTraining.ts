...
export function generateCollegeTrainingBanks(
  ...
  const entries = roster.players
    ...
    .map((player) => {
      ...
      const developmentEffect = staff.get("positionDevelopment") ?? 0.25;
      const strengthEffect = staff.get("strengthCoach") ?? 0.22;
      const recoveryEffect = staff.get("medicalStaff") ?? 0.2;
      ...
      const mentalBank = Math.round(clamp(34 + coachability * 18 + (production?.productionScore ?? player.collegeOverall) * 0.18 + rng.normal(0, 6), 1, 100));
      const fatigue = Math.round(clamp(snapShare * 55 + (player.rosterStatus === "redshirt" ? -12 : 0) + rng.normal(0, 5), 0, 100));
      const recoveryBank = Math.round(clamp(42 + recoveryEffect * 90 - fatigue * 0.25 + rng.normal(0, 5), 1, 100));
      const regressionPressure = Math.round(clamp((player.collegeOverall - player.collegePotential) * 1.8 + fatigue * 0.18 - recoveryBank * 0.08, 0, 100));
      return {
        playerId: player.id,
        ...
        fatigue,
        regressionPressure,
        debug: `${personality.personality}: snap ${Math.round(snapShare * 100)}%, staff dev ${developmentEffect}, strength ${strengthEffect}.`
      };
    });
  return {
    seasonYear,
...
function loadStaffEffects(): Map<string, number> {
  const rows = parseCsv(staffEffectsText);
  const header = rows[0];
  return new Map(rows.slice(1).map((row) => [
    row[header.indexOf("staff_component")],
    Number(row[header.indexOf("effect_weight")]) || 0
  ]));
}

function loadPersonalityArchetypes(): Array<{ personality: string; workEthicMult: number; coachabilityMult: number }> {
